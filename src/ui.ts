...
export class DiffViewer implements Component {
  ...
  private currentSections(): DiffSection[] {
    if (this.mode === "turns") return this.turns[this.turnIndex]?.sections ?? [];
  ...
  private rebuild(): void {
    ...
    for (const section of sections) {
      ...
      const detail = section.detail ? `  ${theme.fg("dim", section.detail)}` : "";
      this.lines.push(`${theme.fg("accent", theme.bold(section.title))}${detail}`);
      if (section.diff) {
        // renderDiff uses pi's active theme, matching inline edit diffs.
        for (const line of renderDiff(section.diff).split("\n")) this.lines.push(line);
      } else {
        this.lines.push(theme.fg("dim", section.note ?? "(no textual diff)"));
      }
      this.lines.push("");
    }
  }
  ...
  render(width: number): string[] {
    ...
    out.push(truncateToWidth(this.theme.fg("dim", `${this.footerHint()}   ${range}`), width));

...
type SnapshotRead = { kind: "text"; text: string } | { kind: "missing" } | { kind: "too-large" };
 ...
 */
export function collectTurns(ctx: ExtensionContext, lookupWrite: WriteDiffLookup): Turn[] {
  ...
  for (const entry of branch) {
    ...
    if (message.toolName === "write") {
      ...
      if (tracked) current?.edits.push({ path: tracked.path, diff: tracked.diff });
    }
  }
